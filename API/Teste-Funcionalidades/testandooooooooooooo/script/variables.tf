
variable "project_name" {
  description = "Project Name - will prefex all generated AWS resource names"
  default     = "devops-sg"
}


provider "aws" {
  profile = "default"
  region  = var.region
  version = "~> 2.61.0"
}


variable "region" {
  default = "ap-southeast-1"
}

data "aws_availability_zones" "azs" {
}



variable "devops_sg_cidr_vpc" {}
variable "key_name" {}
variable "AmazonEC2FullAccess_arn" {arn:aws:iam::aws:policy/AmazonEC2FullAccess}
variable "AmazonS3FullAccess_arn" {arn:aws:iam::aws:policy/AmazonS3FullAccess}
variable "AWSCodedeploy_arn" {arn:aws:iam::aws:policy/AWSCodeDeployFullAccess}
variable "AmazonS3ReadOnlyAccess_arn" {}
variable "sydney_master_key_arn" {}
variable "aws_linux_ami_sg" {}
variable "ssl_cert_arn" {}
variable "ubuntu_18_sg" {}
variable "ubuntu_20_sg" {}
variable "r53zone" {}
variable "externaldnshost" {}
}